...
import { buildChannelChatContent, buildServerUserChatContent, isPlayerLeftEmbedTitle, shouldArchiveServerFromLastEmbedTitles } from "./logger.js";
test("buildChannelChatContent includes username and id", () => {
    assert.equal(buildChannelChatContent("Genesis", "42", "hello world"), "Genesis (42): hello world");
});
test("buildChannelChatContent prefixes radio channel name when provided", () => {
    assert.equal(buildChannelChatContent("Genesis", "42", "radio check", "GlobalRadio"), "[GlobalRadio] Genesis (42): radio check");
});
test("buildServerUserChatContent returns plain-text format for player chat", () => {
    assert.equal(buildServerUserChatContent("Genesis", "hello everyone"), "💬 Genesis: hello everyone");
});
...
test("shouldArchiveServerFromLastEmbedTitles archives only when every latest embed is a leave event", () => {
    assert.equal(shouldArchiveServerFromLastEmbedTitles([
        "📝 Player Left",
        "📝 Player Leaved"
    ]), true);
    assert.equal(shouldArchiveServerFromLastEmbedTitles([
