...
function isAllowedGuild(guildId) {
    ...
    if (!config.guildId) {
        console.error("GUILD_ID is not configured. Ignoring Discord event.");
        return false;
    }
...
client.once(Events.ClientReady, () => {
    ...
    console.log(`Bot restricted to Discord guild: ${config.guildId}`);
    startApi(client);
...
client.on(Events.GuildMemberUpdate, async (oldMember, newMember) => {
    ...
    if (rolesChanged) {
        ...
        if (addedRoles.length) {
            changes.push(`Added roles: ${addedRoles
                .map(id => `<@&${id}>`)
                .join(", ")}`);
        }
        if (removedRoles.length) {
            changes.push(`Removed roles: ${removedRoles
                .map(id => `<@&${id}>`)
                .join(", ")}`);
        }
    }
