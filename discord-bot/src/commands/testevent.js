...
export async function execute(interaction) {
    const channel = interaction.guild?.channels.cache.find(c => c.name === "game-events");
    if (!channel || channel.type !== ChannelType.GuildText) {
        await interaction.reply("❌ #game-events not found");
        return;
    }
    await sendGameEvent(channel, "Player Joined", "TestPlayer");
