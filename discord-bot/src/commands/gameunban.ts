...
export const data = new SlashCommandBuilder()
    .setName("gameunban")
    .setDescription("Unbanning a Roblox player from the experience")
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        await interaction.editReply({
            content: `✅ Queued an unban for ${robloxUsername} for: ${reason}`
        });

    } catch (error) {

        console.error("Failed to queue Roblox unban", error);

        await interaction.editReply({
            content: "⚠️ Failed to queue the unban action."
        });
    }
}
